...
from app.api.supabase import estimate_shipping, ShippingMissing
...
def get_advice(user_input, item_data):
    ...
    if response.api_call == 'none': return ai.invoke_simple(no_call, input_string).content
        ...
        try:
            response = estimate_shipping(response.country, response.weight_g, response.height_cm, response.length_cm, response.width_cm)
            return ai.invoke_simple(shipping_results_received, f'User Message: {user_input}\nShipping Data: {response}')
        except ShippingMissing:
